...
export default function DashboardAssignmentPage({
  ...
  const fetchAssignment = React.useCallback(async () => {
    try {
      ...
      if (mockAssignment) {
        ...
        // 如果有时间限制，设置倒计时
        if (mockAssignment.timeLimit && mockAssignment.timeLimit > 0) {
          setTimeRemaining(mockAssignment.timeLimit * 60); // 转换为秒
        }
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 模拟提交成功
      console.log('Assignment submitted successfully with answers:', answers);
  ...
  const renderQuestion = (question: Question, index: number) => {
    ...
    return (
      <div key={questionId} className="mb-8 p-6 border border-gray-200 rounded-lg">
              ...
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  if (question.maxFileSize && file.size > question.maxFileSize) {
                    alert(`File size exceeds maximum allowed size of ${(question.maxFileSize / 1024 / 1024).toFixed(1)}MB`);
                    return;
                  }
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                ...
                <span>Total Points: {assignment.points}</span>
